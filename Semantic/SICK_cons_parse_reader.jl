...
let
    labels = []
    global spans
    function spans(t::SentimentTree,label=true)
        empty!(labels)
        helper(t,label)
        return labels
    end
...
function build_sick_data(split)
    data=Any[]
    trees = read_file("SICK_cons_parse_"*split*".txt")
    ...
    function build_dict(trn,dev,tst)
        ...
        for s in splits
            for i in 1:length(s)
                tree1=s[i][1]
                tree2=s[i][2]
                push!(words, map(t->lowercase(t.label), leaves(tree1))...)
                push!(words, map(t->lowercase(t.label), leaves(tree2))...)
            end
        end
        words=unique(words)
    ...
    for pair in trn
        for tree in pair
            for leaf in leaves(tree)
                ind = get(w2g, lowercase(leaf.label), randn(Float32,(300,1)))
                leaf.data = ind
            end
